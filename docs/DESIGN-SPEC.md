...
版本：v2 · 2026-09-11  
状态：**当前客户端视觉 SSOT**  
适用范围：Expo / React Native 客户端（大厅、登录、牌桌、人庄、商城、设置）、管理后台、`apps/felt-web` 演示桌，以及共享 Design Tokens。

本规范把 `apps/felt-web` 的深绿毡面语言落到全项目：暗场、椭圆绒面、鼠尾草绿行动色、青柠高光。技术栈仍以 React Native 为主；Web 演示可用 Tailwind / GSAP，token 必须对齐。
...
1. **暗场优先**：全局底 `#07140F`，卡片 `#0E241C`，分区靠间距与细线，不靠浅灰留白。
2. **单一行动色**：鼠尾草绿 `#2E7D63` 用于主按钮、当前行动、入座 CTA。
3. **青柠高光**：`#A3E635` 只用于余额、底池数字、荷官环、当前座位呼吸光，不当大面积底色。
4. **浅字深底**：主文字 `#E8F5EE`，次要文字 `#8FBFAB`。牌面点数例外，仍用深墨 `#17191C` 写在白牌上。
5. **字重分层**：标题 ExtraBold，正文 Regular，数字 SemiBold / ExtraBold。
6. **触控优先**：可点区域 ≥ 44×44；主按钮最小高度 **52**。
7. **荷官顶中**：荷官是发牌起点与视觉锚点，不是装饰人像。
8. **毡面桌，不是浅色棋牌**：椭圆深绿绒面 + 深色木沿成立。仍禁止紫渐变、金色标题当品牌色、写实荷官、WLD 紫。
...
大厅 / 登录 / 商城 / 设置 / 人庄沿用同一暗绿底与深绿卡片，不再使用浅色皮肤。  
Expo `userInterfaceStyle` 与启动屏为 `#07140F`。StatusBar 用 light content。
