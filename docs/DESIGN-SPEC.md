...
版本：v4 · 2026-09-16  
状态：**当前客户端视觉 SSOT**  
适用范围：Expo / React Native **移动端优先**（玩法选择、德州主界面、登录、人庄、商城、设置、6 人桌），管理后台与 `apps/felt-web` 演示桌对齐同一 token。
...
1. **玩法选择（首页 `/`）**：先选玩法，不要在这一页开始匹配或发牌。
2. **德州扑克主界面 `/holdem`**：快速加入、选择公共桌、私人场。再进 6 人桌 `/table`。
3. **人庄主界面 `/stud`**：亮绿毡，直接对庄。

商城、排行榜、设置从玩法选择页进入，**不是**玩法。公共桌列表不再混入人庄入口。
...
| 玩法选择 | `apps/mobile/app/index.tsx` |
| 德州主界面 | `apps/mobile/app/holdem.tsx` |
